...
void increase_occurence_in_file(string &info) {
    // Find the positions of the comma and the closing brace
    size_t commaPos = info.find(",");
    size_t bracePos = info.find("}");

    // Extract the number part of the string
    string numberPart = info.substr(commaPos + 2, bracePos - commaPos - 2);
    ...
    info = info.substr(0, commaPos + 2) + incrementedNumber + "}";
}
...
void adding_each_word(string &currentstring, const string &filename) {
    ...
    if (!currentstring.empty()) {
        ...
        if (binarysearchtree.find(currentstring) == "no_existing_item") {
            ...
            string info = "{ " + filename + ", 1 }";  // if we find a specific word, we will icrement its info according to the file that it exists.
            
            
            binarysearchtree.insert(currentstring);
            hashtable.insert(currentstring);
            
            binarysearchtree.changeDescription(currentstring, info, true);
            hashtable.changeInfo(currentstring, info, true);
            ...
            string info = hashtable.findLatestInfo(currentstring);
            size_t commaPos = info.find(",");
            ...
            if (fileInInfo != filename) {
                ...
                binarysearchtree.changeDescription(currentstring, newFileEntry, true);
                hashtable.changeInfo(currentstring, newFileEntry, true);
            }
                ...
                binarysearchtree.changeDescription(currentstring, info, false);
                hashtable.changeInfo(currentstring, info, false);
 ...
 // time calcualting in one function.

void time_calculating_while_adding_nodes_HASH(vector<WordItem> & hash_node_vector, string & currentstring, long int & hashTableTime){
    ...
    if(currentstring != ""){
        ...
        for (int i = 0; i < k; i++){
            ...
            if (hashtable.find(currentstring) != "no_existing_item"){
                ...
                document = hashtable.findAllInfo(currentstring);
        ...
        worditem.info = document;
        
        hash_node_vector.push_back(worditem);
...
/*
 ...
 It retrieves information about a specific string (currentstring) from the hash table, measures the time taken for this operation, and stores the data in a vector (VEC2).
...
void time_calculating_while_adding_nodes_BST(vector<WordItem> & bst_node_vector, string & currentstring, long int & bstTime){
    ...
    if(currentstring != ""){
        ...
        for (int i = 0; i < k; i++){
            ...
            if (binarysearchtree.find(currentstring) != "no_existing_item"){
                ...
                document = binarysearchtree.AllInfo(currentstring); //this search operation will helps us to understand which data structure is more efficient to use, because the time difference is coming from this operation.
        ...
        bstTime = bstTime + BSTTime.count() / k;
        worditem.info = document;
        bst_node_vector.push_back(worditem); // after the search of a specific key's info, we add the word itself and its corresponding info i.e ({{“a.txt”, 1}, {“b.txt”, 4}}.) to the vector.
...
//new  okok
string print_word_info(const string& filename, const vector<WordItem>& eachode, bool& wordFoundInDocument) {
    ...
    for (const auto& wordInfo : eachode) {
    ...
    while (wordStream.get(currentChar)) {
        ...
        if (islower(currentChar)) { // If it's a lowercase letter
            ...
            if (!currentWord.empty()) {
                ...
                time_calculating_while_adding_nodes_BST(bstData, currentWord, bstTime);
                time_calculating_while_adding_nodes_HASH(hashTableData, currentWord, hashTableTime);
